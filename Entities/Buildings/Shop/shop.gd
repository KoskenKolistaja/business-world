...
func get_sold_products() -> Array:
	var array = []
	for shelf in %ShopShelfContainer.get_children():
		if shelf.shelf_item:
			array.append(shelf.shelf_item)
	return array


func get_shelf_with_item(wanted_item):
	var returned_shelf = null
	for shelf in %ShopShelfContainer.get_children():
		if shelf.shelf_item == wanted_item:
			if shelf.has_shelf_item():
				return shelf
			else:
				returned_shelf = shelf
	return returned_shelf

func get_shop_register():
	return %ShopRegisterContainer.get_children().pick_random()
